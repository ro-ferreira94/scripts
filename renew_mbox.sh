#!/bin/bash
USER=zimbra

mkdir /opt/zimbra/backup/ssl/ || echo "Diretorio ja existe"
echo "Realizando o backup...."
tar -cvf /opt/zimbra/backup/ssl/letsencrypt_$(date +%d_%m_%y).tgz /etc/letsencrypt/live
tar -cvf /opt/zimbra/backup/ssl/zimbra_$(date +%d_%m_%y).tgz /opt/zimbra/ssl
/root/letsencrypt/letsencrypt-auto certonly --http-01-port 81 --standalone -d prodest.correio.es.gov.br -d iases.correio.es.gov.br -d facafacil.correio.es.gov.br

#renew command
cd /etc/letsencrypt/live/prodest.correio.es.gov.br* && \
echo "-----BEGIN CERTIFICATE-----

Ob8VZRzI9neWagqNdwvYkQsEjgfbKbYK7p2CNTUQ
-----END CERTIFICATE-----" >> chain.pem
cp -f /etc/letsencrypt/live/prodest.correio.es.gov.br*/* /opt/zimbra/ssl/letsencrypt
chown $USER:$USER /opt/zimbra/ssl/letsencrypt/*
cat /opt/zimbra/ssl/letsencrypt/cert.pem /opt/zimbra/ssl/letsencrypt/chain.pem >> /opt/zimbra/ssl/letsencrypt/bundle.pem
chown $USER:$USER /opt/zimbra/ssl/letsencrypt/privkey.pem
su - zimbra -c "/opt/zimbra/libexec/zmdomaincertmgr savecrt prodest.es.gov.br /opt/zimbra/ssl/letsencrypt/bundle.pem /opt/zimbra/ssl/letsencrypt/privkey.pem"
su - zimbra -c "/opt/zimbra/libexec/zmdomaincertmgr deploycrts"
su - zimbra -c "zmproxyctl restart"
rm -rf /etc/letsencrypt/live/prodest.correio.es.gov.br*
